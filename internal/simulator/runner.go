...
func NewRunner() (*ConcreteRunner, error) {
	...
	return nil, errors.Wrap(errors.ErrSimulatorNotFound, "Please build it or set ERST_SIMULATOR_PATH")
}
...
func (r *ConcreteRunner) Run(req *SimulationRequest) (*SimulationResponse, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(errors.ErrMarshalFailed, "%w", err)
	}
	...
	if err := cmd.Run(); err != nil {
		...
		return nil, errors.Wrap(errors.ErrSimulationFailed, "%w, stderr: %s", err, stderr.String())
	}
	...
	if err := json.Unmarshal(stdout.Bytes(), &resp); err != nil {
		...
		return nil, errors.Wrap(errors.ErrUnmarshalFailed, "%w, output: %s", err, stdout.String())
	}
	...
	if resp.Status == "error" {
		...
		return nil, errors.Wrap(errors.ErrSimulationLogicError, "%s", resp.Error)
	}
